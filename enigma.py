...
class reflector:
    ...
    # Like the rotor init, but historicReflectorType takes "A", "B", "C".
    def __init__(self, historicReflectorType):
        self.reflectorType = historicReflectorType
        self.permutation = []
        if historicReflectorType == "A":
            self.permutation = [4, 9, 12, 25, 0, 11, 24, 23, 21, 1, 22, 5, 2, 17, 16, 20, 14, 13, 19, 18, 15, 8, 10, 7, 6, 3]
        elif historicReflectorType == "B":
            self.permutation = [24, 17, 20, 7, 16, 18, 11, 3, 15, 23, 13, 6, 14, 10, 12, 8, 4, 1, 5, 25, 2, 22, 21, 9, 0, 19]
        elif historicReflectorType == "C":
            self.permutation = [5, 21, 15, 9, 8, 0, 14, 24, 4, 3, 17, 25, 23, 22, 6, 2, 19, 10, 20, 16, 18, 1, 13, 12, 7, 11]
...
class plugboard:
    ...
    def add(self, l1, l2):
        self.connection[l1] = l2
        self.connection[l2] = l1

    def remove(self, l):
        self.connection[self.connection[l]] = self.connection[l]
        self.connection[l] = l
...
class enigma:
    ...
    def __init__(self, rotorTypes, reflectorType):

        self.rotorQuantity = len(rotorTypes)

        self.rotorBuffer = []
        self.reflector = reflector(reflectorType)
        self.plugboard = plugboard()

        for i in range(self.rotorQuantity):
    ...
    def op(self, text):
        ...
        for l in text:
            ...
            for k in range(self.rotorQuantity):
                ...
                letter = (letter - (rot.rotorPosition - rot.ringPosition))%26

            #Reflect
            letter = self.reflector.permutation[letter]

            #Backward
            for k in range(self.rotorQuantity):
                rot = self.rotorBuffer[k]
                letter = rot.permutationExit[(letter + (rot.rotorPosition - rot.ringPosition))%26]
                letter = (letter - (rot.rotorPosition - rot.ringPosition))%26

            #Plugboard
            letter = self.plugboard.connection[letter]
            newText.append(self.ita(letter))
        return newText
